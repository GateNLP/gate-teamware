...
Some of these may be omitted or others may be required depending on the setup of your specific cluster - see the [chart README](https://github.com/GateNLP/gate-teamware/blob/dev/charts/README.md) and the chart's own values file (which you can retrieve with `helm pull --untar gate/gate-teamware`) for full details.  In particular these values assume:
